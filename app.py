...
def infer_from_audio(text, language, accent, audio_prompt, record_audio_prompt, transcript_content):
    ...
    if audio_prompt is None and record_audio_prompt is None:
        ...
        # tokenize audio
        encoded_frames = tokenize_audio(audio_tokenizer, (wav_pr, sr))
        audio_prompts = encoded_frames[0][0].transpose(2, 1).to(device2)
    ...
    encoded_frames = model.inference(
        text_tokens.to(device2),
        text_tokens_lens.to(device2),
        audio_prompts,
    ...
    samples = vocos.decode(features, bandwidth_id=torch.tensor([2], device=device2))
...
def infer_from_prompt(text, language, accent, preset_prompt, prompt_file):
    ...
    # numpy to tensor
    audio_prompts = torch.tensor(audio_prompts).type(torch.int32).to(device2)
...
def infer_long_text(text, preset_prompt, prompt=None, language='auto', accent='no-accent'):
    ...
    if mode == 'fixed-prompt':
        complete_tokens = torch.zeros([1, NUM_QUANTIZERS, 0]).type(torch.LongTensor).to(device2)
